 ...
 * @version		2.0
 * @filesource	friends/libraries/FriendsCodePack.php
 */

class FriendsCodePack
{
	/**
	...
	public $langItems = array(
		...
		'missing_theme_html_exp'		=> 'There should be a folder called \'html\' inside your site\'s \'/themes/solspace_themes/friends/%code_pack_name%\' folder. Make sure that it is in place and that it contains additional folders that represent the template groups that will be created by this code pack.',
		...
		'missing_code_pack_theme_exp'	=> 'There should be at least one theme folder inside the folder \'%code_pack_name%\' located inside \'/themes/friends/\'. A theme is required to proceed.',
	...
	public function detectInternalCodePack()
	{
		$this->internalCodePack	= ! (
			get_class($this) === 'FriendsCodePack' &&
			//why the need for this? Well sometimes PHP was doing
			// (get_class($this) === 'FriendsCodePack') === true where
		...
		if ( ! empty($folders))
		{
			foreach ($folders as $folder)
			{
				$details = $this->getCodePackDetails(
					$this->addonsFolder . $folder . '/friends/'
				);
	 ...
	 * /some/path/my_sweet_addon/friends/ would return: my_sweet_addon
	 *
	 * @access	public
	...
	public function detectNameFromPath($path = '')
	{
		...
		while ($i--)
		{
			if (trim($segs[$i]) == 'friends' AND $i !== 0)
			{
